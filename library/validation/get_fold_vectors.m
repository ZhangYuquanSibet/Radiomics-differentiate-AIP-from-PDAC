function [d_train, l_train, d_test, l_test] = ...
...
if labels_train(1)==0&&labels_test(1)==0
    num_train_aip = numel(find(labels_train==0));
    num_test_aip = numel(find(labels_test==0));
    d_train = [data_train((num_train_aip+1):end,:);data_train(1:num_train_aip,:)];
    l_train = [labels_train((num_train_aip+1):end,:);labels_train(1:num_train_aip,:)];
    d_test = [data_test((num_test_aip+1):end,:);data_test(1:num_test_aip,:)];
    l_test = [labels_test((num_test_aip+1):end,:);labels_test(1:num_test_aip,:)];
else
    d_train = data_train;
    l_train = lables_train;
    d_test = data_test;
    l_test = labels_test;
end
