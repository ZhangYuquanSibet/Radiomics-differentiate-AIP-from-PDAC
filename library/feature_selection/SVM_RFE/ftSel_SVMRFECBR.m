...
nStopChunk = 100; % when number of features is less than this num, start 
...
if useCBR
	R_all = abs(corr(ft,'type','spearman'));%abs(corrcoef(ft))
end
...
while ~isempty(ftIn)
%     if set==0
% 	    IDX = DR_0(ft(:,ftIn),label);  %%基于类重叠技术选择样本  默认选择0.7
%     elseif set==1
%         IDX = DR_1(ft(:,ftIn),label);  %%基于类重叠技术选择样本  默认选择全部样本
%     elseif set==2
%         IDX = DR(ft(:,ftIn),label);  %%基于类重叠技术选择样本  默认选择0.3~0.7
%     end
	nFtIn = length(ftIn);
...
if kerOpt.type==2
   type= ['-t ',num2str(kerOpt.type),'-c ',num2str(kerOpt.C),' -g ',num2str(kerOpt.g)];
%    type = sprintf('-s 0 -t %d -c %f -g %f -q',kerOpt.type,kerOpt.C,kerOpt.g);
elseif kerOpt.type==0
    type= ['-t ',num2str(kerOpt.type),'-c ',num2str(kerOpt.C)];
end
model = lib_svmtrain(Y, X, type);
